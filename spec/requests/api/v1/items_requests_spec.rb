...
# require 'pry'; binding.pry
  ...
  it 'finds all items based on min price' do
    ...
    expect(items.second[:attributes][:unit_price]).to eq(item_3.unit_price)
    # This passes in my Rspec test, but fails the Postman because it is returning items formatted the same way as the find by (params[:name]) action.
  end
